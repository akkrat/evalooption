...
from approach_eval.core import ROOT, CACHE, PYTHON, config, digest, dump, git, init_snapshot, snapshot, fingerprint
from approach_eval.grading import changes, grade
from approach_eval.workflows import TOOLING, install, stages
...
def run_one(task, approach, directory, settings=None, recover_from=None):
    ...
    resume_nodes = dict(recovery["resume_nodes"]) if recovery else {}
    status = "running"
    error = None
    final_grade = None
    ...
    def agent_loop(prompt, stage, session=None, depth=0):
        ...
        node = resume_nodes.pop(stage, None)
        if node:
            session = node["session"]
            if node["child_stage"]:
                child = agent_loop(common + "\nResume only your existing interrupted worker assignment from this session. Finish it and return its requested handoff.", node["child_stage"], depth=depth+1)[1]
                prompt = common + "\nFresh isolated worker responses (continue your orchestration):\n" + json.dumps([child])
            elif node.get("pending_response", {}).get("status") == "ask":
                answer = user_turn(node["pending_response"]["message"], "question", stage)
                prompt += "\nUser answer:\n" + answer["message"]
            ...
            if response["status"] == "dispatch":
                ...
                for worker_prompt in response["dispatches"]:
                    ...
                    reply = agent_loop(
                        ...
                        stage + f"-worker{(recovery['previous_dispatch_count'] if recovery else 0) + codex.dispatch_count}", depth=depth + 1)[1]
    ...
    try:
        ...
        if recovery:
            common += ("\nAuthorized additional-budget continuation. The workspace is now " + str(candidate) +
                ". Use this checkout exclusively. Old absolute checkout paths are inaccessible; previous process handles are no longer live. "
                "No hidden tests, reference solution, independent review, or evaluator feedback is supplied. "
                "Continue your existing assignment and do not repeat completed stages or answered questions.\nOriginal task:\n" + initial["message"])
        for stage, template, review in stages(approach):
            ...
            if recovery:
                prompt += ("\nContinuing a budget-exhausted run from its existing artifacts and original sessions. "
    ...
    result = dict(task=task["id"], complexity=task["complexity"], approach=approach,
                  ...
                  study_kind="additional-budget continuation; incremental metrics only",
