...
await page.getByRole('combobox',{name:'Forecast range',exact:true}).selectOption('next-year');
const report=page.getByTestId('balance-forecast-report');
const assertMinimum=async (amount,date)=>{
 await expect(report.getByTestId('balance-forecast-lowest').getByText(amount,{exact:true}).first()).toBeVisible();
 await expect(report.getByTestId('balance-forecast-lowest').getByText(date,{exact:true})).toBeVisible();
};
...
await page.getByRole('combobox',{name:'Forecast range',exact:true}).selectOption('next-3-months');
...
await page.reload();
await expect(page.getByRole('combobox',{name:'Forecast range',exact:true})).toHaveValue('next-3-months');
await expect(page.getByTestId('balance-forecast-report')).toContainText('Daily points include');
...
await expect(page.getByText('Dashboard widget successfully saved.',{exact:true}).first()).toBeVisible();
await page.reload();
await expect(page.getByTestId('balance-forecast-report')).toContainText('Monthly points are calendar month-end snapshots.');
