...
from approach_eval.core import ROOT, PYTHON, command, dump
from runtime import NODE, CACHE, environment
...
Only solve the supplied task. Use local loopback for preview/test servers only. Do not access the internet, other runs, original fixes,
...
your work with available tests. Use the pinned Node/Yarn runtime and installed dependencies; do not install dependencies. All process invocations must remain within the sandbox.
"""
...
class Codex:
    ...
    def call(self, role, cwd, prompt, session=None, stage="", read_only=False, artifact_only=False, _retry=0):
        ...
        actor_env.update(environment(cwd))
        actor_env["PATH"] = actor_path
        effort = self.settings["user_effort" if role == "user" else "agent_effort"]
        args = [shutil.which("codex") or "codex", "exec", "--cd", str(cwd)]
        if session:
            args += ["resume", session]
        ...
        fs = settings["permissions"]["evaluation"]["filesystem"]
        fs[str(CACHE / "actual-playwright")] = "read"
        fs[str(CACHE / "actual-yarn-cache")] = "read"
        fs[str(CACHE / "actual-instrumentation")] = "read"
        for dep in [cwd / "node_modules", *cwd.glob("packages/*/node_modules")]:
            fs[str(dep)] = "read"
            if role == "agent" and not read_only and not artifact_only:
                for name in (".vite", ".vite-temp", ".cache"):
                    fs[str(dep / name)] = "write"
        if role == "agent":
            fs[str(cwd / "node_modules/.cache/actual-eval-tmp")] = "write"
            fs[str(cwd / ".eval-output")] = "write"
            settings["features.network_proxy"] = True
            settings["permissions"]["evaluation"]["network"] = {
                "enabled": True, "allow_upstream_proxy": False,
                "allow_local_binding": True,
                "domains": {"localhost": "allow", "127.0.0.1": "allow"}}
        broker = BrowserBroker(cwd, readonly=read_only or artifact_only) if role == "agent" else None
        if broker:
            actor_env["ACTUAL_BROWSER_RUNNER"] = broker.url
            settings["developer_instructions"] += (
                "\nBrowser verification: macOS blocks direct Chromium launch inside the CLI. "
                "Use the harness browser runner: write a .js/.mjs/.cjs script inside this workspace "
                "(for example .eval-output/browser.cjs), then POST JSON {script: relative_path, timeout: 120} "
                "to the ACTUAL_BROWSER_RUNNER environment URL using Python urllib. It runs Node with "
                "Playwright/Chromium, the same workspace, no reference files, and loopback-only network. "
                "Response JSON contains returncode, stdout, stderr and output_directory. "
                "Save screenshots under ACTUAL_BROWSER_OUTPUT, which is set inside the runner. "
                "The script may create a local server, run browser checks, and must close browser/server. "
                "Direct Chromium launch is unsupported; do not spend time debugging its macOS crash.")
            settings["features.view_image"] = True
