...
- [OpenSpec](https://github.com/Fission-AI/OpenSpec): CLI 1.12.0. The harness uses the package's actual generated Codex skills and validators. Discovery reference checkout commit is saved in `tasks/workflows.lock.json`; npm dependency resolution is saved in the committed `tasks/tooling-package-lock.json`.
...
Both external workflows are installed from the committed npm lock during bootstrap. No private project checkout or previously generated study result is required for a new library evaluation.
